...
class EMNISTActionDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, A=2, cyclic=False, transform=None, exclude_pairs={}):
        ...
        for idx, (image, label) in enumerate(self.dataset):
            ...
            label = int(label) - 10
            if label >= N_DIGITS or label < 0:
                continue
            self.data_idx_by_class[label].append(len(self.images))
    ...
    def __getitem__(self, idx):
        ...
        if self.only_zero_action:
            ...
            target_label = (label + action) % N_DIGITS
        else:
            max_label = N_DIGITS - 1
            action_list = action_list[
                (action_list >= max(-label, -self.A))
                & (action_list <= min(max_label - label, self.A))
            ]
            action = random.choice(action_list)
...
def _run_single_task(args):
    ...
    out_dir = f"figures_EMNIST_cDCGAN/A_{A}/seed_{seed}"
    ...
    excluded_pairs_accuracy = (
        test_model_excluded_pairs(
            ...
            latent_dim,
            device,
            f"{out_dir}/samples_excluded_pairs.png",
            n_samples=10,
        )
        if dataset.exclude_pairs
        else None
    )
