...
class Generator(nn.Module):
    ...
    def forward(self, image, action):
        z = self.encoder(torch.cat([image, action], dim=1))
        out = self.decoder(z)
        return out
...
if __name__ == "__main__":
    ...
    dataset = MNISTActionDataset(A=1, transform=transform)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for real_imgs, labels, target_imgs, target_labels, actions in train_loader:
            ...
            # Train Generator
            # ------------------------
            gen_imgs = G(real_imgs, actions.unsqueeze(1))
        ...
        with torch.no_grad():
            samples = G(real_imgs, actions.unsqueeze(1)).view(-1, 1, 28, 28)
