...
def test_model_excluded_pairs(G, cond_encoder, dataset, classifier, latent_dim, device, save_path, n_samples=10):
    ...
    mean_distance = abs(pred - target_labels).float().mean().item()
...
def save_trial_statistics(G, cond_encoder, train_loader, latent_dim, device, save_path, classifier=None, excluded_pairs_accuracy=None, excluded_pairs_mean_distance=None):
    ...
    if excluded_pairs_mean_distance is not None:
        stats["excluded_pairs_mean_distance"] = excluded_pairs_mean_distance
    ...
    excluded_pairs_mean_distance_str = f" (excluded pairs mean distance: {excluded_pairs_mean_distance:.4f})" if excluded_pairs_mean_distance is not None else ""
    print(f"Saved trial statistics to {save_path}{acc_str}{excluded_pairs_acc_str}{excluded_pairs_mean_distance_str}")
    ...
    for A_dir in sorted(glob.glob(os.path.join(base_folder, "A_*"))):
        ...
        for seed_dir in sorted(glob.glob(os.path.join(A_dir, "seed_*"))):
            if not os.path.isdir(seed_dir):
                continue
            print(f"loading trial statistics for {seed_dir}")
...
def _run_single_task_on_gpu(gpu_id, A, seed):
    ...
    if exclude_pairs is not None:
        excluded_pairs_accuracy, excluded_pairs_mean_distance = test_model_excluded_pairs(G, cond_encoder, dataset, classifier, latent_dim, device, f"{out_dir}/samples_excluded_pairs.png", n_samples=10)
    else:
        excluded_pairs_accuracy = None
        excluded_pairs_mean_distance = None
    save_trial_statistics(G, cond_encoder, test_loader, latent_dim, device, f"{out_dir}/trial_statistics.pkl", classifier=classifier, excluded_pairs_accuracy=excluded_pairs_accuracy, excluded_pairs_mean_distance=excluded_pairs_mean_distance)
